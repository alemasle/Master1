...
public class ASD {
	...
	static public class ProtoFonction {
		...
		String type;
		String name;
		List<ParamProto> params;

		public ProtoFonction(String type, String name, List<ParamProto> params) {
			this.type = type;
			this.name = name;
			this.params = params;
		}
		...
		public RetProtoFonction toIR(SymbolTable ts) throws TypeException {
			...
			if (ts.lookup(name) != null && ts.lookup(name) instanceof FunctionSymbol) {
				...
				for (ParamProto p : params) {
					ParamProto.RetParamProto retP = p.toIR(ts);
					para = retP.varSym;
	...
	static public class Fonction {
		...
		public Fonction(String type, String name, List<ParamProto> params, FonctionCorps c) {
			this.type = type;
			this.name = name;
			this.params = params;
			this.c = c;
		...
		public RetFonction toIR(SymbolTable ts) throws TypeException {
			...
			if (name.compareTo("main") == 0) {
				...
				if (ts.lookup(name) != null && ts.lookup(name) instanceof FunctionSymbol) {
					...
					SymbolTable args = new SymbolTable(ts);
			...
			for (ParamProto p : params) {
				ParamProto.RetParamProto tmp = p.toIR(ts);
				pS.add(tmp.name);
			}
	...
	static public class ParamProto {

		Type type;
		String name;

		public ParamProto(Type type, String name) {
			this.type = type;
			this.name = name;
		}
		...
		static public class RetParamProto {
			// The LLVM IR:
			public Llvm.IR ir;
			public String name;
			public SymbolTable.VariableSymbol varSym;

			public RetParamProto(Llvm.IR ir, String name, VariableSymbol varSym) {
				this.ir = ir;
				this.name = name;
				this.varSym = varSym;
			}
		...
		public RetParamProto toIR(SymbolTable ts) throws TypeException {

			Llvm.IR paramIR = new Llvm.IR(Llvm.empty(), Llvm.empty());

			SymbolTable.VariableSymbol varS = new VariableSymbol(type, name);
			ts.add(varS);

			return new RetParamProto(paramIR, name, varS);
	...
	static public class FonctionCorps {
		...
		public RetFonctionCorps toIR(SymbolTable ts, FunctionSymbol fct) throws TypeException {
			...
			SymbolTable fctTable = new SymbolTable(fct.arguments);

			Bloc.RetBloc retB = b.toIR(fctTable, fct);
	...
	static public class CallInt extends Expression {
		...
		public RetExpression toIR(SymbolTable ts) throws TypeException {
			...
			Llvm.Instruction call = new Llvm.CallFonctionInt(t.toLlvmType(), name, pS, result);
