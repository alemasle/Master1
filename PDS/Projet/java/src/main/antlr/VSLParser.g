...
program returns [ASD.Program out]
	:{List <ASD.ProtoFonction> lfproto = new ArrayList<ASD.ProtoFonction>();
	  ...
	  List <ASD.ParamProto> lpr = new ArrayList<ASD.ParamProto>();
	  List <ASD.ParamProto> lp = new ArrayList<ASD.ParamProto>();
	  String type = "";}
	
	( PROTO	( DECINT {type = "INT";} | VOID {type = "VOID";} )	IDENT LP (pro=paramsproto {lpr.add($pro.out);} (VIRGULE pro=paramsproto {lpr.add($pro.out);})* )? RP
	{ ASD.ProtoFonction protoF = new ASD.ProtoFonction(type, $IDENT.text, lpr); lfproto.add(protoF);} 
	)*
	
	( FUNC ( DECINT {type = "INT";} | VOID {type = "VOID";} ) IDENT LP (p=paramsproto {lp.add($p.out);} (VIRGULE p=paramsproto {lp.add($p.out);})* )? RP c=corps
...
paramsproto returns [ASD.ParamProto out]
	: IDENT { $out = new ASD.ParamProto(new ASD.IntType(), $IDENT.text);}
	;
...
appelfonctionvoid returns [ASD.Instructions out]
	...
	IDENT LP (p=params {lp.add($p.out);} (VIRGULE p=params {lp.add($p.out);})* )? RP 
